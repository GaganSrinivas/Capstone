...
def swap_piece(im, pos, true_pos, cuts=2, dim=200):
    ...
    piece = copy.copy(im[x * cut_len:(x + 1) * cut_len,
                      y * cut_len:(y + 1) * cut_len, ])
    swap_piece = copy.copy(im[x_true * cut_len:(x_true + 1)
                           * cut_len, y_true * cut_len:(y_true + 1) * cut_len, ])

    im[x * cut_len:(x + 1) * cut_len, y * cut_len:(y + 1)
       * cut_len, ] = swap_piece
    im[x_true * cut_len:(x_true + 1) * cut_len, y_true *
       cut_len:(y_true + 1) * cut_len, ] = piece
...
def extract_piece(a, size=200, cuts=2):
    ...
    if cuts == 3:
        a = np.array([a[:, 0:cut_len, :], a[:, cut_len:cut_len *
                     2, :], a[:, cut_len * 2:cut_len * 3, :]])
        a = np.concatenate(
